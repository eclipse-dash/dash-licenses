...
public class MavenIdParser implements ContentIdParser {
	...
	private static final String MAVEN_PATTERN = "(?<groupid>[^: ]+)" + ":(?<artifactid>[^: ]+)"
			+ "(:(?<ext>[^: ]*)(:(?<classifier>[^: ]+))?)?"
			+ ":(?<version>\\d+(\\.\\d+){0,2})(?:\\.(?<qualifer>\\d{8,}))?[^: ]*" + "(:(?<phase>[^:]*))?";
	...
	public Optional<IContentId> parseId(String value) {
		...
		// think is
		// important, I don't think that we can automagically map this to actual valid
		// Maven
		// coordinates, but we may be able to map it to something that we can match with
		// EF data. We might, for example, try to do some sort of bundle mapping with
		// (for this
		// particular example, CQ 11159.

		// We originally implemented this using Aether, but Aether breaks when a phase
		// is
		// included in the value. So, we brute force it with a regular expression. While
		// we're here
